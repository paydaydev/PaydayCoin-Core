...
static int totalPaydayCoinCheckCount = 0;
static int totalPaydayCoinCheckMicros = 0;
...
#if defined(NDEBUG)
#error "PaydayCoin cannot be compiled without assertions."
#endif
...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& view, const CTxMemPool& pool, unsigned int flags, bool cacheSigStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck>* pvChecks, std::vector<CScriptCheckConcurrent>* pvChecksConcurrent, uint256* hashCacheEntry) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    AssertLockHeld(cs_main);
 ...
 */
static bool AcceptToMemoryPoolWorker(const CChainParams& chainparams, CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx, bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced, bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool bMultiThreaded, bool bSanityCheck) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
        ...
        for (const CTxIn& txin : tx.vin) {
            ...
            if (ptxConflicting) {
                if (!setConflicts.count(ptxConflicting->GetHash())) {
                    ...
                    if (fEnableReplacement && tx.nVersion != PAYDAYCOIN_TX_VERSION_ASSET_ALLOCATION_SEND) {
...

    {
        ...
        if (!bMultiThreaded) {
            ...
            if (!CheckPaydayCoinInputs(false, tx, state, view, true, bOverflow, ::ChainActive().Height(), CBlock(), bSanityCheck)) {
                return error("mandatory-paydaycoin-inputs-check-failed (%s)", state.GetRejectReason());
            }
        }
        if (test_accept) {
        ...
        for (CTxMemPool::txiter it : allConflicting) {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s PDAY additional fees, %d delta bytes\n",
        ...
        if (bMultiThreaded && threadpool != NULL) {
            ...
            std::packaged_task<void()> task([&pool, chainparams, txIn, hash, coins_to_uncache, hashCacheEntry, vChecksConcurrent]() {
                ...
                int thisPaydayCoinCheckCount = 0;
                int thisCheckMicros = 0;
                int thisPaydayCoinCheckMicros = 0;
                ...
                for (const auto& check : vChecksConcurrent) {
                    ...
                    if (!isCheckPassing) {
                        ...
                        LogPrint(BCLog::MEMPOOL, "%s: %s\n", "CheckPaydayCoinInputs Sig Error", hash.ToString());
                        {
                            LOCK2(cs_main, mempool.cs);
                            for (const COutPoint& hashTx : coins_to_uncache)
                                pcoinsTip->Uncache(hashTx);
                            pool.removeConflicts(txIn);
                            pool.removeRecursive(txIn, MemPoolRemovalReason::PAYDAYCOIN);
                ...
                if (isCheckPassing) {
                    ...
                    int64_t paydaycoinCheckTime;
                    if (fLogThreadpool) {
                        paydaycoinCheckTime = GetTimeMicros();
                        thisPaydayCoinCheckCount += 1;
                    }
                    {
                        bool bOverflow = false;
                        if (!CheckPaydayCoinInputs(false, txIn, validationState, coinsViewCache, true, bOverflow, ::ChainActive().Height(), CBlock())) {
                            nLastMultithreadMempoolFailure = GetTime();
                            LogPrint(BCLog::MEMPOOL, "%s: %s\n", "CheckPaydayCoinInputs Error", hash.ToString());
                            {
                                LOCK2(cs_main, mempool.cs);
                                for (const COutPoint& hashTx : coins_to_uncache)
                                    pcoinsTip->Uncache(hashTx);
                    ...
                    if (fLogThreadpool) {
                        // how long did we run CheckPaydayCoinInputs()'s
                        thisPaydayCoinCheckMicros = GetTimeMicros() - paydaycoinCheckTime;
                    }
                }
                if (fLogThreadpool) {
                    // metrics
                    ...
                    totalPaydayCoinCheckCount += thisPaydayCoinCheckCount;
                    totalPaydayCoinCheckMicros += thisPaydayCoinCheckMicros;
            ...
            if (fLogThreadpool && totalCheckCount > 0 && totalPaydayCoinCheckCount > 0 && totalExecutionCount > 0 && (!fUnitTest || (fUnitTest && totalExecutionCount % 100 == 0))) {
                int avgCheckMicros = totalCheckMicros / totalCheckCount;
                int avgPaydayCoinCheckMicros = totalPaydayCoinCheckMicros / totalPaydayCoinCheckCount;
                ...
                LogPrint(BCLog::THREADPOOL, "THREADPOOL::%s:Signature check internals - check(%d): total %lld, avg %lld microseconds, paydaycoin(%d): total %lld, avg %lld microseconds\n", hash.ToString(), totalCheckCount, totalCheckMicros, avgCheckMicros, totalPaydayCoinCheckCount, totalPaydayCoinCheckMicros, avgPaydayCoinCheckMicros);
...
static bool AcceptToMemoryPoolWithTime(const CChainParams& chainparams, CTxMemPool& pool, CValidationState& state, const CTransactionRef& tx, bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced, bool bypass_limits, const CAmount nAbsurdFee, bool test_accept, bool bMultiThreaded = false, bool bSanityCheck = true) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    // PAYDAYCOIN if its been less 60 seconds since the last MT mempool verification failure then fallback to single threaded
    if (GetTime() - nLastMultithreadMempoolFailure < 60) {
        ...
        bMultiThreaded = false;
    } else if (!fConcurrentProcessing || test_accept || tx->nVersion != PAYDAYCOIN_TX_VERSION_ASSET_ALLOCATION_SEND)
        bMultiThreaded = false;
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransactionRef& tx, bool* pfMissingInputs, std::list<CTransactionRef>* plTxnReplaced, bool bypass_limits, const CAmount nAbsurdFee, bool test_accept, bool bMultiThreaded, bool bSanityCheck) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...
CAmount GetBlockSubsidy(unsigned int nHeight, const Consensus::Params& consensusParams, CAmount& nTotalRewardWithMasternodes, bool fSuperblockPartOnly, bool fMasternodePartOnly, unsigned int nStartHeight)
{
    if (nHeight == 0)
        return 0 * COIN;
    if (nHeight == 1) {
        // PAYDAYCOIN 4 snapshot
        nTotalRewardWithMasternodes = 8000000 * COIN;
    ...
    const CAmount& nSuperblockPart = (nSubsidy * 0.0);
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (!passetdb || !passetallocationdb) {
        error("DisconnectBlock(): PaydayCoin dbs do not exist");
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        // PAYDAYCOIN
        if (!DisconnectPaydayCoinTransaction(tx, pindex, view, mapAssets, mapAssetAllocations, vecMintKeys))
            fClean = false;
    }
    // PAYDAYCOIN
...
bool CChainState::ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    bool bOverflow = false;
    // PAYDAYCOIN
    if (!CheckPaydayCoinInputs(IsInitialBlockDownload(), *block.vtx[0], state, view, fJustCheck, bOverflow, pindex->nHeight, block))
        return error("ConnectBlock(): CheckPaydayCoinInputs on block %s failed\n", block.GetHash().ToString());
    ...
    // PAYDAYCOIN : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
...
bool CChainState::ActivateBestChain(CValidationState& state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock)
{
    ...
    // PAYDAYCOIN Notify external listeners about accepted block header
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    int nHeight = (::ChainActive().Tip()!=NULL ? ::ChainActive().Tip()->nHeight : 0);
    if (nHeight > Params().Masternode_Fork_Block()) {
        CTransactionRef tx = block.vtx[0];
        if (tx->vout.size() < 2) {
            return error("%s : rejected by check masternode payment lock-in at %d", __func__, nHeight);
        }

        CAmount nTotalRewardWithMasternodes;
        CAmount blockReward = GetBlockSubsidy(nHeight, Params().GetConsensus(), nTotalRewardWithMasternodes);
        int nIndex = tx->vout.size() - 1;
        if (tx->vout[1].nValue < blockReward * 0.05) {
            return error("%s : rejected by check masternode payment lock-in at %d", __func__, nHeight);
        }
    }
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (!consensusParams.AllowLegacyBlocks(nHeight) && block.IsLegacy())
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, error("%s : legacy block after auxpow start", __func__),
